...
CREATE TABLE IF NOT EXISTS titan.medications(
  ...
  route TEXT,
  freq TEXT,
  status TEXT NOT NULL DEFAULT 'current', -- current|prn|stopped
  recorded_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS titan.labs(
  ...
  user_id UUID NOT NULL REFERENCES titan.users(user_id) ON DELETE CASCADE,
  test_code TEXT,             -- e.g., LOINC/alias (A1C, PSA, UACR, eGFR)
  test_name TEXT NOT NULL,    -- human label
  value TEXT NOT NULL,
  unit TEXT,
  ref_range TEXT,
  result_date DATE NOT NULL,
  source_note UUID,           -- FK to titan.notes.note_id (nullable)
  recorded_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS titan.procedures(
  ...
  user_id UUID NOT NULL REFERENCES titan.users(user_id) ON DELETE CASCADE,
  proc_type TEXT NOT NULL,     -- colonoscopy, eye_exam, etc
  proc_date DATE,
  result_text TEXT,
  recorded_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS titan.cardio_tests(
  ...
  user_id UUID NOT NULL REFERENCES titan.users(user_id) ON DELETE CASCADE,
  modality TEXT NOT NULL,      -- echo, stress, ekg
  test_date DATE,
  ef_percent NUMERIC,          -- echo EF if present
  findings TEXT,               -- free text summary
  rbbb BOOLEAN,                -- ECG finding
  lbbb BOOLEAN,
  av_block TEXT,               -- e.g., first-degree
  axis TEXT,                   -- left/right/normal
  recorded_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Latest-value helper views
